...
1. A **filtered per-timepoint table** that every downstream analysis (Behavioral, Death & Morphology, Interaction, Active Killing) actually reads.
2. A **summarised per-track table** with one row per surviving track. *Currently consumed only by the legacy DTW-based T-cell analysis (see [below](#what-the-summarised-csv-is-for)).*
...
All filters are **independent** — turn any combination on. When a filter is unchecked, its spinner is greyed out and the filter is skipped.
...
Same columns as the input Feature Extraction CSV, with only the rows of surviving tracks (subject to experiment duration and max-length trims). One row per `(track, timepoint)`. **This is the file every downstream analysis reads** — Behavioral, Death & Morphology, Interaction, Active Killing all start from here.
...
- [Behavioral Analysis](behavioral) — consumes the filtered per-timepoint CSV.
- [Death & Morphology Analysis](death_morphology) — consumes the filtered per-timepoint CSV.
