...
| 4 | 📍 **Tracking** | Per cell-type subtabs: btrack (objects that do not overlap between frames), fragmentation tracking (objects that do), Reporter Propagation, LAP, TrackPy, or Import existing. Manual editing of tracked segments lives in the Visualization tab. | [Tracking](processing/tracking/index) |
