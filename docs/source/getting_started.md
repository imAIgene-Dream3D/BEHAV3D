...
The launcher reads `napari/behav3d_env.json` (created by the installer), activates the right conda env, starts napari, and adds the BEHAV3D EXPLORER dock widget on the right side of the viewer.
...
| 3 | 🦠 **Segmentation** | Five methods: APOC, ConvPaint, Pixel Classifier, Cellpose, Import existing. | [Segmentation](processing/segmentation/index) |
...
3. **Segment in Segmentation** ([details](processing/segmentation/index))
   - Pick a method.
   - Train (Pixel Classifier / APOC / ConvPaint) or load a pretrained model (Cellpose), segment training data to check the result in Visualization, then batch the rest via the Processing Queue.
