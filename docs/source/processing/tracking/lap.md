...
A general centroid-distance tracker. Consider it only when you want **globally optimal** linking (the whole cost matrix solved at once, unlike [TrackPy](trackpy)'s greedy nearest-neighbour), **gap closing** across brief dropouts, or built-in **division / fusion** modelling. For fast or crowded cells, **[btrack](btrack)** disambiguates crossing tracks better.
...
the plugin squares your value for you before calling `laptrack` — so if you cross-reference the
[laptrack docs](https://laptrack.readthedocs.io/) (where `track_cost_cutoff` is a squared
distance), don't square it yourself in the field. Linking uses the full **3-D** centroid distance
in physical units (z scaled by `pixel_distance_z`).
```

| Field | Default | Range | Effect |
|---|---|---|---|
| **Track cost** | 45 | 1–999999 | Maximum distance a cell can move between two **consecutive** frames to be linked as the same track. |
| **Gap close cost** | 60 | 1–999999 | Maximum distance allowed when reconnecting a track across **one or more missing** frames. Should be ≥ Track cost. |
...
| **Merging cost** | 0 | 0–999999 | Maximum distance for detecting merge events (two tracks → one). `0` disables merging detection. |
| **Splitting cost** | 0 | 0–999999 | Maximum distance for detecting split events (one track → two). `0` disables splitting detection. |
...
8. (Optional) **Use the Apply buttons** to propagate these settings to other cell types with similar dynamics: **Apply to all *Category*** or **Apply to all**.
9. **Click *Run *Cell type* Tracking*** (the green button at the bottom of the sub-tab, named after the cell type) for an immediate run across all samples. To run every cell type at once, or to queue the work, use the tab-level **Run Batch Tracking (All Cell Types)** button or the **+🛒** queue button below the sub-tabs — see [Generic workflow](./index.md#generic-workflow-per-sub-tab).
...
- **Start with the defaults and look at the result before tuning.** The defaults (Track cost 45, Gap close 60, Gap frames 3) are a generic starting point for moderate cell motion, not a tuned recommendation. Tune whenever you see specific failure modes.
