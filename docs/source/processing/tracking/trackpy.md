...
[TrackPy](http://soft-matter.github.io/trackpy/) is a particle-tracking library originally written for colloidal-particle tracking (Crocker-Grier algorithm). In BEHAV3D EXPLORER it provides a **simpler and faster alternative to LapTrack** for sparser data — when cells are well-separated and you don't need built-in merge / split modelling.
...
| **Memory (frames)** | 2 | 0–100 | Number of frames a cell can disappear and still be reconnected to its previous track — analogous to LapTrack's gap-closing max frames. |
...
3. **Estimate the fastest single-frame displacement in your data** (same logic as LapTrack — load a sample in Visualization, scrub two consecutive frames, eyeball the fastest cell's step).
4. **Set `Search range`** ≈ that displacement, in the unit selected in the **Distance units** toggle (µm by default; default value 31).
5. **Set `Memory`** = how many consecutive missing frames a cell can have and still be reconnected (default 2). This is the analogue of LapTrack's gap closing.
...
- Fix: **lower `Search range`**. If that fragments other tracks, the dataset is genuinely ambiguous — consider LapTrack (global optimisation) or btrack (motion model).
...
- Fix: **lower `Search range`** to the smallest value that still produces continuous tracks for the fastest cells. If you can't lower it without fragmenting tracks, consider switching to LapTrack or btrack.
...
- TrackPy has no native merge / split detection. Switch to LapTrack (with `Merging cost` / `Splitting cost` > 0) or [btrack](btrack).
...
- **`Memory` recovers brief segmentation gaps**, similar to LapTrack's gap closing. Start at 2; raise it if you see TrackID flipping when a cell briefly disappears.
- **No native merge / split detection.** If cells fuse or divide in your dataset, prefer [LapTrack](lap) (with merging / splitting cost > 0) or [btrack](btrack) — and for motile cells generally, prefer btrack regardless.
...
- That TrackPy is faster than LapTrack in every case — for very sparse data it generally is, but for moderately dense data, LapTrack's global optimisation can produce better tracks at comparable speed. Measure on your own data.
...
- [LapTrack](lap) for dense data and merge/split modelling.
