...
for t, t_dying_orgs in tqdm(enumerate(dying_orgs), total = dying_orgs.shape[0]):
    ...
    col_dict = dict(zip(t_df['TrackID'], t_df["percentage_dead_mask"]))
...
viewer.add_image(raw_image[:,0], name='Raw Image (T-cell)', colormap= "cyan", scale=elsize, blending='additive')
viewer.add_image(raw_image[:,1], name='Raw Image (Organoid)', colormap= "yellow", scale=elsize, blending='additive')
viewer.add_image(raw_image[:,2], name='Raw Image (Death)', colormap= "red", scale=elsize, blending='additive')
