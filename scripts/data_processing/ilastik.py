...
def run_ilastik_pixel_classifier(raw_data, model, output_dir=None):
...
def run_ilastik_object_classifier(raw_data, pixel_probabilities, model, output_dir=None):
    if output_dir is None:
        out_path = Path(Path(raw_data).parent, f"{Path(raw_data).stem}_preobjects.h5")
        out_feature_csv_path = Path(Path(raw_data).parent, f"{Path(data_path).stem}_preobject_features.h5")
    else:
        out_path = Path(output_dir, f"{Path(raw_data).stem}_preobjects.h5")
        out_csv_path = Path(output_dir, f"{Path(data_path).stem}_preobject_features.h5")
...
def run_ilastik_object_splitter(raw_data, segments, model, output_dir=None):
    if output_dir is None:
        out_path = Path(Path(raw_data).parent, f"{Path(data_path).stem}_finalobjects.tiff")
    else:
        out_path = Path(output_dir, f"{Path(data_path).stem}_finalobjects.tiff")
