...
def _resolve_track_paths(output_dir, cell_type, output_subdir_name="behavioral_tracks"):
    """Resolve canonical track-classification paths under behavioral_states."""
    ...
    outfolder = state_outdir / str(output_subdir_name)
...
def _resolve_track_classifier_path(output_dir, cell_type, output_subdir_name="behavioral_tracks"):
...
def train_track_classifier(
    ...
    if model_adata is None:
        ...
        if verbose:
            print(f"Loaded model adata from: {model_adata_path}")
    ...
    if verbose and validation_mode == "holdout":
        ...
        print(
            "Classifier holdout validation: "
            f"n_train={classifier_validation['n_train']}, n_test={classifier_validation['n_test']}, "
        ...
        tm = classifier_validation["train_metrics"] or {}
        print(
            "Classifier OOB-only validation: "
    ...
    artifact = {
        ...
        "track_filtering": dict(model_adata.uns.get("track_filtering", {})),
        "track_feature_selection": dict(model_adata.uns.get("track_feature_selection", {})),
    ...
    if bool(save_classifier):
        ...
        if verbose:
            print(f"Saved track classifier artifact: {save_path}")
...
def apply_track_classifier_to_subtracks(
    ...
    min_length = int(filter_cfg.get("min_length", 100))
    max_length = int(filter_cfg.get("max_length", 100))
...
def export_track_cluster_backprojection(
    ...
    """
    Export behavioral-state backprojections using track-cluster labels.
    """
    labeled_full = assign_track_clusters_to_full_dataset(
        adata_full=adata_full,
        adata_tracks=adata_tracks,
        cluster_col=cluster_col,
        output_col=output_col,
        id_cols=(sample_col, track_col),
        inplace=False,
    )

    adata_export = labeled_full
    if sample_name is not None and len(str(sample_name).strip()) > 0:
        selected = str(sample_name).strip()
        sample_values = labeled_full.obs[sample_col].astype("string").fillna("")
        mask = sample_values == selected
        if int(mask.sum()) == 0:
            raise ValueError(
                f"Could not export track-cluster backprojection: sample '{selected}' not found in adata_full."
            )
        adata_export = labeled_full[mask.to_numpy()].copy()
    ...
    if plot_exemplars:
        ...
        if bool(autosave_plots) and fig_exemplar is not None:
            ...
            exemplar_path = Path(
                ...
                f"exemplar_tracks_{_sanitize_filename_token(cluster_key)}_state_{_sanitize_filename_token(state_col)}.pdf",
            )
